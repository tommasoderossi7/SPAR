...
def _extract_external_base_fields(data: Dict[str, Any]) -> Tuple[str, str]:
    # STRICT: must have prompt and base completion
    prompt = None
    if isinstance(data.get("prompt"), str):
        prompt = data["prompt"]
    if not prompt and isinstance((data.get("metadata") or {}).get("prompt"), str):
        prompt = data["metadata"]["prompt"]
    if not prompt and isinstance((data.get("problem") or {}).get("prompt"), str):
        prompt = data["problem"]["prompt"]
    if not prompt:
        raise RuntimeError(
            "External base: prompt not found and --external-use-prompt required."
        )

    base_text = None
    base_text = data.get("solution") or {}
    if not base_text:
        raise RuntimeError("External base: base completion not found.")
...
async def build_token_steps_via_iterative_lcp(
    ...
    *,
    ...
    """
    Build per-token next-token distributions aligned to the external base completion
    by iteratively re-prompting with the already-matched prefix.

    NEW: If an alignment step makes no progress (k == 0), retry the *same prefix*
    up to 20 times, printing each attempt. If still no progress after 20 tries,
    raise a RuntimeError. No other fallbacks.
    """
    # Tokenize the *external* base once (ids + offsets in ext_completion_raw)
    ...
    # Use the *sentence-level* sampling params you specified
    align_temp = float(args.external_temp)
    align_top_p = float(args.external_top_p)

    while matched < N:
        # Prompt = external prompt + the already-matched piece of the base completion
        ...
        for attempt in range(1, 21):  # 1..20
            ...
            if response.get("error"):
                # This is still a hard failure (you asked only to retry on k==0)
                raise RuntimeError(
            ...
            if k > 0:
                ...
                print(
                    f"[Align p{problem_idx}] attempt {attempt}/20 @token {matched}/{N}: "
        ...
        # We have k > 0 and 'last_entries' contains the entries that made progress
        # Keep ONLY the first k entries, and rewrite indices & absolute char offsets
        for j in range(k):
            e = last_entries[j]
            abs_t = matched + j
            lp = e.get("logprob")
            ...
            token_steps.append(
                {
                    ...
                    "text_offset": int(
                        ext_offs[abs_t][0]
                    ),  # absolute offset in ext_completion_raw
                }
            )

        matched += k

    # Done: aligned entries for *all* tokens of the external base
    base_outcome = default_outcome_extractor(_clean_token_display(ext_completion_raw))
    is_correct = False  # caller will check against GT if provided
    return {
...
async def generate_rollout_resume(
    ...
    *,
    ...
    # 1) Base path: STRICT external-driven LCP alignment (no fallback)
    if "base" not in result or "token_steps" not in result:
        # Locate and read the external sentence-level artefact
        ...
        # Prompt & base completion (strict)
        ext_prompt, ext_completion_raw = _extract_external_base_fields(ext_data)
        if not args.external_use_prompt:
            raise RuntimeError(
                "--external-use-prompt must be set (no fallback logic allowed)."
            )

        # Store prompt exactly as external
        result["prompt"] = ext_prompt
        safe_write_json(rollout_file, result)

        # Build aligned token steps by iterative LCP calls
        aligned = await build_token_steps_via_iterative_lcp(
            problem_idx,
            ext_prompt,
            ext_completion_raw,
            semaphore=semaphore,
        ...
        # Optional correctness check if GT is present
        base_outcome = aligned["base_outcome"]
        is_correct = False
        gt = (result.get("problem") or {}).get("gt_answer")
        if gt and base_outcome and base_outcome != "__empty__":
            is_correct = check_answer(base_outcome, gt)
        ...
        result["base"] = {
            ...
            "source": "external_sentence_level",
        }

        result["token_steps"] = aligned["token_steps"]
