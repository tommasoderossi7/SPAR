...
parser.add_argument("--use-prob-true", action="store_true", default=False)
...
def _normalize_weights(pairs: Iterable[Tuple[str, float]]) -> Dict[str, float]:
    # pairs: (token_string, weight)
    d = {}
    s = 0.0
    for k, w in pairs:
...
def _empirical_dist_from_samples(samples: List[Dict[str, Any]]) -> Dict[str, float]:
    # outcome histogram from branch samples
    ...
    if tot == 0:
        return {}
    return {k: v / float(tot) for k, v in counts.items()}


def _kl_bern(p_true_p: float, p_true_q: float, alpha_small: float = 1e-9) -> float:
    # tiny smoothing like the chunk code
    p = min(max(p_true_p, alpha_small), 1 - alpha_small)
    q = min(max(p_true_q, alpha_small), 1 - alpha_small)
    return p * math.log(p / q) + (1 - p) * math.log((1 - p) / (1 - q))


def _kl_ansdist(
    P: Dict[str, float], Q: Dict[str, float], alpha_small: float = 1e-9
) -> float:
    keys = set(P) | set(Q)
    if not keys:
        return 0.0
    # tiny smoothing
    sum_p = sum(P.get(k, 0.0) for k in keys) + alpha_small * len(keys)
    sum_q = sum(Q.get(k, 0.0) for k in keys) + alpha_small * len(keys)
    kl = 0.0
    for k in keys:
        p = (P.get(k, 0.0) + alpha_small) / sum_p
        q = (Q.get(k, 0.0) + alpha_small) / sum_q
        kl += p * math.log(p / q)
    return max(0.0, kl)


def _prob_true_from_dist(dist: Dict[str, float], gt: Optional[str]) -> float:
    if not gt or not dist:
        return 0.0
    p = 0.0
    for ans, mass in dist.items():
        if isinstance(ans, str) and isinstance(mass, (int, float)):
            try:
...
def compute_token_importance_metrics_inplace(
    result_obj: Dict[str, Any],
    *,
    use_prob_true: bool = True,
) -> None:
    """
    Adds result_obj['token_importance'] = [{
        't': int,
        'text_offset': int,
        'token': str,
        'p_greedy': float,
        'acc_greedy': float,
        'acc_altmix': float,
        'delta_acc': float,          # alt - greedy
        'kl_token': float,           # KL(alt || greedy) over outcomes or Bernoulli P(true)
        'n_alt_branches': int,
        'n_greedy_samples': int,
        'n_alt_samples': int
    }, ...]
    Only positions with ≥1 alternate branch and ≥1 greedy sample are included.
    """
    problem = result_obj.get("problem", {}) or {}
    gt = problem.get("gt_answer")
    ...
    for entry in steps:
        ...
        p_star = entry.get("probability") or 0.0

        # Map token -> branch (samples)
        br_map = {br.get("token"): br for br in branches if isinstance(br, dict)}
        br_star = br_map.get(w_star)

        # alt weights from recorded candidate probs
        alts_raw = []
        for br_tok, br in br_map.items():
            if br_tok == w_star:
                continue
            pw = br.get("probability")
            if isinstance(pw, (int, float)) and pw > 0:
                alts_raw.append((br_tok, float(pw)))
        w_alt = _normalize_weights(alts_raw)
        if not w_alt or (not br_star):
            continue

        # build dist_greedy from samples; fallback to base outcome if totally empty
        dist_greedy = _empirical_dist_from_samples(br_star.get("samples"))
        if not dist_greedy:
            base_outcome = (result_obj.get("base") or {}).get("outcome")
            dist_greedy = {base_outcome: 1.0} if base_outcome else {}

        # build alt mixture over branches (exclude greedy)
        mix = defaultdict(float)
        n_alt_samples = 0
        for tok, w in w_alt.items():
            br = br_map.get(tok)
            if not br:
                continue
            dist_w = _empirical_dist_from_samples(br.get("samples"))
            for a, m in dist_w.items():
                mix[a] += w * m
            n_alt_samples += len(br.get("samples") or [])
        # renormalize alt mixture
        s = sum(mix.values())
        dist_alt = {k: v / s for k, v in mix.items()} if s > 0 else {}

        # compute accuracy delta & KL
        acc_g = _prob_true_from_dist(dist_greedy, gt)
        acc_a = _prob_true_from_dist(dist_alt, gt)

        if use_prob_true:
            kl = _kl_bern(acc_a, acc_g)
        else:
            kl = _kl_ansdist(dist_alt, dist_greedy)

        out.append(
            {
                "t": entry.get("token_index"),
                "text_offset": entry.get("text_offset"),
                "token": w_star,
                "p_greedy": float(p_star) if isinstance(p_star, (int, float)) else 0.0,
                "acc_greedy": float(acc_g),
                "acc_altmix": float(acc_a),
                "delta_acc": float(acc_a - acc_g),  # same sign as chunk metric
                "kl_token": float(kl),
                "n_alt_branches": len(w_alt),
                "n_greedy_samples": len(br_star.get("samples") or []),
                "n_alt_samples": int(n_alt_samples),
            }
        )

    result_obj["token_importance"] = out
    # document how we computed it
    result_obj.setdefault("importance_meta", {})["token_use_prob_true"] = use_prob_true
...
async def generate_rollout_resume(
    ...
    *,
    ...
    # 6) Token-level importance (apples-to-apples with chunk code)
    #    Set use_prob_true=True to match the default in your chunk analysis (args.use_prob_true defaulted True there).
    compute_token_importance_metrics_inplace(result, use_prob_true=args.use_prob_true)
