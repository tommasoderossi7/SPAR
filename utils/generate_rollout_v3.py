...
def build_outcome_distributions(
    base_outcome: str, token_entries: List[Dict[str, Any]]
    ...
    for entry, tail_logprob in zip(token_entries, tail_sums):
            ...
            for sample in branch.get("samples", []):
                ...
                if continuation_probability is None:
                    ...
                    continuation_probability = (
                        math.exp(lp_sum) if isinstance(lp_sum, (int, float)) else 0.0
                    )
                distribution_weights[
                    str(entry.get("token_index", -1))
                    + "__"
                    + str(sample.get("sample_index", -1))
        ...
        normalized = (
            {k: v / total_weight for k, v in distribution_weights.items()}
            if total_weight > 0
            else {}
        )
...
def compute_drift_series(
    base_distribution: Dict[str, float], distributions: List[Dict[str, Any]]
) -> List[Dict[str, Any]]:
...
async def generate_rollout(
    ...
    print(
        f"Identified {potential_forking_tokens} potential forking tokens across {potential_forking_positions} positions."
    )
    print(
        f"Sampling {args.samples_per_fork} continuations per alternate token ({potential_forking_tokens * args.samples_per_fork} calls)."
    )
