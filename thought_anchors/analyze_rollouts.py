...
parser.add_argument(
    "-bs", "--batch_size", type=int, default=8192, help="Batch size for embedding model"
)
parser.add_argument(
    "-us",
    "--use_similar_chunks",
...
def calculate_counterfactual_importance_kl(
    chunk_idx, chunk_info, chunk_embedding_cache, chunk_accuracies, chunk_answers, args
):
    """
    ...
    for answer, count in similar_answers.items():
        for _ in range(count):
            similar_sols.append(
                {"answer": answer, "is_correct": answer_correctness.get(answer, False)}
            )
...
def calculate_resampling_importance_kl(chunk_idx, chunk_info, problem_dir):
    ...
    return calculate_kl_divergence(
        chunk_sols1, chunk_sols2, use_prob_true=args.use_prob_true
    )
...
def calculate_forced_importance_kl(
    ...
    return calculate_kl_divergence(
        current_solutions, next_solutions, use_prob_true=args.use_prob_true
    )
...
def analyze_problem(
    ...
    if sentence_model not in embedding_model_cache:
        embedding_model_cache[sentence_model] = SentenceTransformer(sentence_model).to(
            "cuda:0"
        )
